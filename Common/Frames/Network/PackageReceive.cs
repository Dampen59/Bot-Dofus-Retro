...
{
    public static class PackageReceive
    {
        ...
        public static void Inicializar()
        {
            ...
            foreach (MethodInfo tipo in asm.GetTypes().SelectMany(x => x.GetMethods()).Where(m => m.GetCustomAttributes(typeof(PackageAttribut), false).Length > 0))
            {
                ...
                metodos.Add(new PackageData(instancia, atributo.package, tipo));
            }
        }

        public static void Recibir(ClienteTcp cliente, string package)
        {
            PackageData metodo = metodos.Find(m => package.StartsWith(m.nombre_package));
